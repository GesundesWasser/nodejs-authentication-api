  ...
  db.get(
    ...
    async (err, row) => {
      ...
      try {
        ...
        if (!response.ok) {
          logger.error("TinySRC API error", {
            url,
          ...
          return res
            .status(500)
            .json({ error: data.error || "Shortening failed" });
        }
          ...
          function (err) {
            if (err) {
              if (err.message.includes("UNIQUE")) {
                logger.warn("Shortlink race condition, fetching existing", {
                  url,
                });
                return db.get(
                  `SELECT shortUrl FROM shortlinks WHERE longUrl=?`,
                  ...
                  (err2, row2) => {
                    if (err2) {
                      logger.error("Shortlink fallback DB error", {
                        url,
                        error: err2.message,
                      });
                      return res.status(500).json({ error: "Database error" });
              ...
              logger.error("Shortlink insert DB error", {
                url,
                error: err.message,
              });
              return res.status(500).json({ error: "Database error" });
