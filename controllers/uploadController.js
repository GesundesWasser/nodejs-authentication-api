...
router.post("/", auth, adminOnly, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (mime === "image/svg+xml") {
      ...
      logger.debug("SVG read complete", {
        originalName,
        outputSizeKB: (buffer.length / 1024).toFixed(1),
      });
      ...
      logger.debug("Image conversion complete", {
        originalName,
        outputSizeKB: (buffer.length / 1024).toFixed(1),
      });
      ...
      logger.debug("Video conversion complete", {
        originalName,
        outputSizeKB: (buffer.length / 1024).toFixed(1),
      });
      fs.unlink(mp4Path, () => {});
    ...
    res
      .status(500)
      .json({ error: "Processing or upload failed: " + err.message });
  } finally {
    fs.unlink(tmpPath, () => {});
  }
});
