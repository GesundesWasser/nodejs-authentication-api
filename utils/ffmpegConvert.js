...
function videoToMP4(inputPath) {
    return new Promise((resolve, reject) => {
        ...
        ffmpeg(inputPath)
            ...
            .output(outPath)
            .on("start", (cmd) => logger.debug("ffmpeg video command started", { cmd }))
            .on("progress", (p) => logger.debug("ffmpeg video progress", { percent: p.percent?.toFixed(1) }))
            .on("end", () => {
                logger.debug("Video to MP4 conversion complete", { outPath });
                resolve(outPath);
            })
            .on("error", (err) => {
                logger.error("Video to MP4 conversion failed", { inputPath, error: err.message });
                reject(err);
            })
            .run();
    });
...
function audioToMP3(inputPath) {
    return new Promise((resolve, reject) => {
        ...
        ffmpeg(inputPath)
            ...
            .output(outPath)
            .on("start", (cmd) => logger.debug("ffmpeg audio command started", { cmd }))
            .on("end", () => {
                logger.debug("Audio to MP3 conversion complete", { outPath });
                resolve(outPath);
            })
            .on("error", (err) => {
                logger.error("Audio to MP3 conversion failed", { inputPath, error: err.message });
                reject(err);
            })
            .run();
    });
